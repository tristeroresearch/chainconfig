...
const argv = yargs(hideBin(process.argv))
    ...
    .option('chains', {
        type: 'string',
        describe:
            'Comma/space separated chain keys from chainconfig to probe (uses their rpcUrls).',
    })
    .option('all-chains', {
        type: 'boolean',
        default: false,
...
const checkRpc = async (url, timeoutMs) => {
    ...
    try {
        ...
        if (data?.error) {
            ...
            return {
                status: 'supported',
                message: `Error response (likely due to params): ${message}`,
            };
        }
...
async function runChainMode(chains) {
    console.log(
        `Checking eth_callMany support across ${chains.length} chain(s) from chainconfig...`,
    );
    console.log(`Timeout per request: ${argv.timeout} ms\n`);
    ...
    for (const chain of chains) {
        ...
        for (const rpc of rpcUrls) {
            ...
            if (result.status === 'supported') {
                ...
                console.log(
                    `${prefix} ❌ ${rpc} does NOT support eth_callMany (${result.message})`,
                );
            } else {
    ...
    if (missing.length > 0) {
        ...
        missing.forEach((entry) => {
            const reason =
                entry.status === 'no-rpcs' ? 'no RPC URLs configured' : 'no supporting RPCs found';
