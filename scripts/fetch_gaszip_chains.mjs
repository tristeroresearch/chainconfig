...
function deriveApiUrl(explorerUrl) {
    ...
    const patterns = [
        ...
        {
            match: /https:\/\/(.*?)scan\.io/,
            replace: (m, chain) => `https://api.${chain}scan.io/api`,
        },
        {
            match: /https:\/\/(.+?)\.etherscan\.io/,
            replace: (m, chain) => `https://api-${chain}.etherscan.io/api`,
        },
        {
            match: /https:\/\/explorer\.(.+?)\.io/,
            replace: (m, chain) => `https://api.explorer.${chain}.io/api`,
        },
        {
            match: /https:\/\/blockscout\./,
            replace: (m) => `${explorerUrl.replace(/\/$/, '')}/api`,
        },
    ];

    for (const { match, replace } of patterns) {
        if (match.test(explorerUrl)) {
            return typeof replace === 'function' ? explorerUrl.replace(match, replace) : replace;
        }
    }
